...
class Simulation:
    ...
    def parse_conf(self, conf_fn, prediction):
        ...
        self.sys_fail = System_Failure(self.env, self.sys_tbf_distr, self.sys_nodes, self.node_failures, self.soft_failures, prediction)
...
if __name__ == "__main__":
    ...
    for i in range(sim_runs):
        ...
        sim.parse_conf('sim_conf.xml', prediction)
        sim.run()
        ...
        for app in sim.apps:
            ...
            if (len(app.ckpt_2bb_percentages) == 0):
                avg_percentage_of_ckpt_2bb = 0
            else:
                avg_percentage_of_ckpt_2bb = mean(app.ckpt_2bb_percentages)
            if (len(app.ckpt_intervals) == 0):
                avg_ckpt_interval = 0
            else:
            ...
            f.write('App total write workloads to BB: ' + str(app.id) + ','+ app.name + ',' + str(sim.bb.get_total_write_workload(app.name) / 1024) + '\n')
            f.write('App total read workloads from BB: ' + str(app.id) + ',' + app.name + ',' + str(sim.bb.get_total_read_workload(app.name) / 1024) + '\n')
            f.write('App Write workloads to BB per day: ' + str(app.id) + ',' + app.name + ',' + str(sim.bb.get_total_write_workload(app.name) / app.comp_intvs[-1][1] * 24 / 1024) + '\n')
