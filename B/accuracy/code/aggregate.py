    ...
    for line in f:
        ...
        if a[0] == 'App Write workloads to BB per day':
            ...
            if b[1] in sum_app_failures:
                ...
                sum_app_no_of_ckpts[b[1]] = float(b[2])
        elif a[0] == 'App comp saved time ckpt':
            b = a[1].split(',')
            if b[1] in sum_app_comp_ckpt_saved_time:
                sum_app_comp_ckpt_saved_time[b[1]] += float(b[2])
            else:
                sum_app_comp_ckpt_saved_time[b[1]] = float(b[2])
        elif a[0] == 'App comp saved time live migration':
            b = a[1].split(',')
            if b[1] in sum_app_comp_lm_saved_time:
                sum_app_comp_lm_saved_time[b[1]] += float(b[2])
            else:
                sum_app_comp_lm_saved_time[b[1]] = float(b[2])
...
for k in sum_app_total_time.keys():
    ...
    print ('Average efficiency of '+k+': '+str(sum_app_efficiency[k]/sim_runs * 100))
    print ('Average failures of '+k+': '+str(sum_app_failures[k]/sim_runs))
    print('Average comp time saved ckpt of ' + k + ': ' + str(sum_app_comp_ckpt_saved_time[k] / sim_runs))
    print('Average comp time saved live migration of ' + k + ': ' + str(sum_app_comp_lm_saved_time[k] / sim_runs))
