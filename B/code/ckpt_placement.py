...
class Checkpoint_Placement:
    ...
    def optimization_bb_only_no_limit(self, apps, sys_fail, bb, pfs, current_time):
        ...
        for app in apps:
            ...
            #print(app.name, app_ckpt2pfs_time[var_id], app_ckpt2bb_time[var_id], app_fail_rates[var_id])
            app.comp_period = math.sqrt((2 * app_ckpt2bb_time[var_id] / app_fail_rates[var_id]) + (2 * app_ckpt2pfs_time[var_id] * app_ckpt2bb_time[var_id]))
            #print(app.name, 'new', app.comp_period)
            #print(app.name, 'old', math.sqrt((2 * (app_ckpt2bb_time[var_id] + app_ckpt2pfs_time[var_id]) / app_fail_rates[var_id])))
    ...
    def optimization_bb_only_with_limit(self, apps, sys_fail, bb, pfs, current_time):
        last_fail_time = sys_fail.get_last_fail_time()
        sys_fail_rate = (integrate.quad(lambda x: sys_fail.get_tbf_distr().hazard(x), current_time - last_fail_time,
                                        current_time - last_fail_time + self.updt_window)[0]) / self.updt_window
        print('average system failure rate during update window: ' + str(sys_fail_rate))
        ...
        for k, v in var_id_2_app_name.items():
            for app in apps:
                if app.name == v:
                    bb_wrt_limits = bb.get_wrt_lim_per_day(app.clients)
                    #print (app.name)
                    print (app.name + 'comp_period before: '+str(app.comp_period))
                    obj_func = lambda x: app_left_comp_time[k] *\
                                         (1 + ((app_ckpt2pfs_time[k] + app_ckpt2bb_time[k])/ x) + (0.5 * app_fail_rates[k] * x))
                    cons_func = lambda x:((app_ckpt_size[k] / x)) - (bb_wrt_limits * app_clients[k]/ 24.0)

                    x0 = math.sqrt(2 * (app_ckpt2bb_time[k] + app_ckpt2pfs_time[k]) / app_fail_rates[k])
                    contol = 1e-5
                    gtol = 1e-5
                    p = NLP(obj_func, x0, c=cons_func, gtol=gtol, contol=contol, iprint=10, maxIter=10000, maxFunEvals=1e7, name='NLP_ckpt_plcmnt')
                    solver = 'ralg'
                    r = p.solve(solver, plot=0)
                    #print(r.xf)
                    app.comp_period = r.xf
                    app.ckpt_intervals.append(app.comp_period)
                    app.ckpt_2bb_percentages.append(1)
                    print (app.name + 'comp_period after: '+str(app.comp_period))
                    print (app.name + 'comp_period after: ' + str(math.sqrt(2 * (app_ckpt2bb_time[k] + app_ckpt2pfs_time[k]) / app_fail_rates[k])))
    ...
    def run(self, apps, sys_fail, bb, pfs):
        while True:
            with self.resource.request(priority=-1) as req:
                ...
                #self.optimization_bb_only_with_limit(apps, sys_fail, bb, pfs, current_time)
