...
class Simulation:
    ...
    def parse_conf(self, conf_fn, fpp, tpp, fnp):
        ...
        self.sys_fail = System_Failure(self.env, self.sys_tbf_distr, self.sys_nodes, self.node_failures, self.soft_failures, self.live_migration_threshold, fpp, tpp, fnp)
...
if __name__ == "__main__":
    ...
    res_path = sys.argv[3]
    fpp = float(sys.argv[4])
    tpp = float(sys.argv[5])
    fnp = float(sys.argv[6])
    ...
    for i in range(sim_runs):
        ...
        sim.parse_conf('sim_conf.xml', fpp, tpp, fnp)
        sim.run()
