...
class System_Failure:
    def __init__(self, env, tbf_distr, nodes, node_failures, soft_failures, live_migration_threshold, falsepositivepct, truepositivepct, falsenegativepct):
        self.env = env
        ...
        self.multi_failure_pct = 20
        self.multi_failure = False
        self.multi_failure_sequence = False
    ...
    def run(self, apps, bb, pfs):
        while True:
            ...
            for app_proc in self.app_procs:
                if app_proc.get_exe().is_alive and app_proc.get_app().client_belong_to(self.fail_client_id):
                    if (self.latest_failure.truepositive == True and self.latest_leadtime / 3600 < app_proc.get_app().live_migration_threshold) or self.latest_failure.falsenegative == True:
    ...
    def run1(self, apps, bb, pfs):
        while True:
            ...
            #print('failure model: waiting till the alarm time:', self.env.now + tbf - (self.latest_failure.meanleadtime / 3600))
            ...
            if self.latest_failure.meanleadtime / 3600 < self.live_migration_threshold:
                for app_proc in self.app_procs:
                    if app_proc.get_exe().is_alive and app_proc.get_app().client_belong_to(self.fail_client_id):
                        app_proc.get_exe().interrupt('failure:' + str(self.fail_client_id))
                        failed_app = app_proc.get_app()
                        self.fail_time.append(self.env.now)
                        self.failures_generated.append(self.latest_failure)
                        self.fail_client_ids.append(failed_app.id)
