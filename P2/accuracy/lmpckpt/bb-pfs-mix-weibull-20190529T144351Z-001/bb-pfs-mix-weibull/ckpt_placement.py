...
class Checkpoint_Placement:
    def __init__(self, env, resource, updt_window, live_migration_failure_pct):
        self.env = env
        self.resource = resource
        self.updt_window = updt_window
        self.live_migration_failure_pct = live_migration_failure_pct
    ...
    def optimization_bb_only_no_limit(self, apps, sys_fail, bb, pfs, current_time):
        ...
        for app in apps:
            ...
            sys_fail_rate = (1 - app.live_migration_percentage) * (
                integrate.quad(lambda x: sys_fail.get_tbf_distr().hazard(x), current_time - last_fail_time,
            ...
            if app_fail_rates[var_id] == 0:
                app.ckpt_intervals.append(1)
                app.ckpt_2bb_percentages.append(0)
                app.comp_period = 1
            else:
    ...
    def optimization(self, apps, sys_fail, bb, pfs, current_time):
        ...
        sys_fail_rate = (1 - self.live_migration_failure_pct)*(integrate.quad(lambda x: sys_fail.get_tbf_distr().hazard(x), current_time-last_fail_time, current_time-last_fail_time+self.updt_window)[0])/self.updt_window
