...
class Simulation:
    ...
    def parse_conf(self, conf_fn, lmsize_var):
        [self.sys_tbf_distr, self.sys_nodes, self.apps, self.bb, self.pfs, self.ckpt_plcmnt, self.node_failures,
         self.soft_failures, self.live_migration_threshold] = self.conf_parser.parse(conf_fn, self.env, self.resource)
        self.sys_fail = System_Failure(self.env, self.sys_tbf_distr, self.sys_nodes, self.node_failures, self.soft_failures, self.live_migration_threshold)
        self.app_procs = [Scientific_App_Proc(self.env, app, self.bb, self.pfs, self.sys_fail, lmsize_var) for app in self.apps]
...
if __name__ == "__main__":
    ...
    for i in range(sim_runs):
        ...
        sim.parse_conf('sim_conf.xml', lmsize_var)
        sim.run()
        ...
        for app in sim.apps:
            #print(app.name)
            #print('comp intervals', app.comp_intvs)
            ...
            #print(len(app.comp_saved_lm_intvs))
            #print(len(app.comp_saved_ckpt_intvs))
            comp_saved_time_ckpt = sum([x[1] - x[0] for x in app.comp_saved_ckpt_intvs])
            comp_saved_time_lm = sum([x[1] - x[0] for x in app.comp_saved_lm_intvs])
            ...
            f.write('App comp saved time ckpt: ' + str(app.id) + ',' + app.name + ',' + str(comp_saved_time_ckpt) + '\n')
            f.write('App comp saved time live migration: ' + str(app.id) + ',' + app.name + ',' + str(comp_saved_time_lm) + '\n')
