...
class Configuration_Parser:
    ...
    def parse(self, conf_fn, env, resource):
        ...
        live_migration_threshold = float(root.find('sys_fail').find('live_migration_threshold').text)
        total_failure_count = 0
        total_failure_live_migration_count = 0
        for node_failure in root.find('node_failures').findall('node_failure'):
            ...
            meanleadtime = float(node_failure.find('meanleadtime').text) * 1.4
            if (meanleadtime / 3600) >= live_migration_threshold:
                total_failure_live_migration_count += frequency
            total_failure_count += frequency
        ...
        for soft_failure in root.find('soft_failures').findall('soft_failure'):
            ...
            meanleadtime = float(soft_failure.find('meanleadtime').text) * 1.4
            if (meanleadtime / 3600) >= live_migration_threshold:
                total_failure_live_migration_count += frequency
            total_failure_count += frequency
        ...
        #live_migration_failure_pct = float(root.find('sys_fail').find('live_migration_failure_pct').text)
        live_migration_failure_pct = total_failure_live_migration_count / total_failure_count

        #print('live_migration_failure_pct', live_migration_threshold)
        ...
        ckpt_placement = Checkpoint_Placement(env, resource, ckpt_plcmnt_updt_window, live_migration_failure_pct)

        return [sys_tbf_distr, sys_nodes, apps, burst_buf, parallel_file_sys, ckpt_placement, node_failures, soft_failures, live_migration_threshold]
